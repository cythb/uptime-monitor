...
const introComment = async () => `# This file was generated by cythb/uptime-monitor@${await exports.getUptimeMonitorVersion()}
#
# ===============================
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Generate graphs
        uses: cythb/uptime-monitor@${await exports.getUptimeMonitorVersion()}
        with:
          command: "graphs"
        env:
...
const setupCiWorkflow = async () => {
    const config = await config_1.getConfig();
    var ghRepo = `${config.owner}/${config.repo}`;
    if (config["status-website"] && config["status-website"].ghOwner && config["status-website"].ghRepo) {
        ghRepo = `${config["status-website"].ghOwner}/${config["status-website"].ghRepo}`;
    }
    const workflowSchedule = config.workflowSchedule || {};
